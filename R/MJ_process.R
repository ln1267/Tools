...
#--------mean for each basins
lin<-ANN_COM_new[c(1,11:36)]
lin<-melt(lin,id=c(1,10,11))
Mean_basin<-dcast(lin,Basin~variable,mean,na.rm=TRUE)
write.csv(Mean_basin,"Mean_basin.csv")


ANN_all<-ANN_COM_new[c(19,20,21,23,31,34,35,36)]

lin<-melt(ANN_all,id=c(1,2))
lin_ann<-dcast(lin,YEAR~CELL+variable)
...
f_trend<-function(n){
  ...
  return(c(n-1,a$sl,a$B))
}

library(doParallel)
cl<-makeCluster(detectCores()-1)
clusterExport(cl,c("lin_ann"))
tr<-parLapply(cl,c(2:130201),f_trend)

tr<-lapply(c(2:108501),f_trend)

ts<-do.call(rbind,tr)
a<-ts[,2]
P_vars<-matrix(a,ncol=6,byrow=TRUE)
a<-ts[,3]
Slope_vars<-matrix(a,ncol=6,byrow=TRUE)

summary(P_vars)
summary(Slope_vars)

a<-subset(ANN_COM_new,YEAR==2002)

Trend_all<-cbind(a[1:9],P_vars,Slope_vars)


names(Trend_all)[10:21]<-c("P_pre","p_ET","P_gep","P_RE","P_NEP","P_Q","Slope_pre","Slope_ET","Slope_gep","Slope_RE","Slope_NEP","Slope_Q")

rm(a1,a2)

plot_f<-function(name1){
  
  gplot<- ggplot(aes(x = LONG, y = LAT), data = Trend_all) +
    geom_raster(aes(LONG, LAT, fill=P_Q))+
    #  facet_wrap( ~ YEAR, ncol=2)+
    scale_fill_gradient(limits=c(0,0.05),low = 'red', high = 'green',name="name1",na.value = "grey70") +
  ...
  gplot<- ggplot(aes(x = LONG, y = LAT), data = Trend_all) +
    geom_raster(aes(LONG, LAT, fill=Slope_gep))+
    #  facet_wrap( ~ YEAR, ncol=2)+
    scale_fill_gradient(low = 'red', high = 'green',name="",na.value = "grey70") +
    coord_equal(xlim=c(102.5, 104.0),ylim = c(30.72,33.18))+
    labs(x="Latitude (°)",y="Longitude (°)")+ 
    theme_grid()
  
  ggsave(gplot,file =paste("trend/","GEP.pdf",sep=""),width = 10,  units = c("cm"),dpi = 300)
  
  
}

theme_grid <- function(base_size = 8, base_family = "Times"){
...
lin<-data.frame(ID=ANN_COM_new$CELL,YEAR=ANN_COM_new$YEAR,ET_P=ANN_COM_new$AET/ANN_COM_new$Pre,PET_P=ANN_COM_new$PET/ANN_COM_new$Pre)

lin$YEAR<-as.factor(lin$YEAR)
lin$`ANN_COM_new$LC_2002`<-as.factor(lin$`ANN_COM_new$LC_2002`)
lin<-cbind(lin,ANN_COM_new$Basin,ANN_COM_new$LC_2002)


##--- trend
lin1<-melt(Trend_all[c(1,16:21)],id=c(1))
lin1<-dcast(lin1,Basin~variable,mean,na.rm=TRUE)

Trend_all$VEG<-as.factor(Trend_all$VEG)
levels(Trend_all$VEG)[levels(Trend_all$VEG)=="1"] <- "Evergreen needleleaf forest"
levels(Trend_all$VEG)[levels(Trend_all$VEG)=="4"] <- "Deciduous broadleaf forest"
levels(Trend_all$VEG)[levels(Trend_all$VEG)=="5"] <- "Mixed forest"
levels(Trend_all$VEG)[levels(Trend_all$VEG)=="7"] <- "Grasslands"
levels(Trend_all$VEG)[levels(Trend_all$VEG)=="8"] <- "Croplands"

lin1<-melt(Trend_all[c(3,16:21)],id=c(1))
lin1<-dcast(lin1,VEG~variable,mean,na.rm=TRUE)

#---compare different vegetation type

lin<-melt(mean_ann_MJ_Veg,id=c(1,2))
lin<-dcast(lin,YEAR~VEG+variable)
n<-2
f_mk<-function(n){
  a<-ts(lin[[n]])
  a1<-sens.slope(a)
  a2<-mk.test(a)
  return(c(n-1,a1$b.sen,a2$pvalg))
}

MK_veg<-lapply(c(2:26),f_mk)
MK_veg<-do.call(rbind,MK_veg)
MK_veg<-as.data.frame(MK_veg)
MK_veg<-cbind(MK_veg,names=names(lin)[2:26])
