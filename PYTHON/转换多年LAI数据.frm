VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} 转换多年LAI数据 
   Caption         =   "UserForm1"
   ClientHeight    =   3180
   ClientLeft      =   45
   ClientTop       =   375
   ClientWidth     =   4710
   OleObjectBlob   =   "转换多年LAI数据.frx":0000
   StartUpPosition =   1  '所有者中心
End
Attribute VB_Name = "转换多年LAI数据"
    ...
    For j = 1 To 12
        For h = VEG_start To VEG_num
            ...
            If (Sheet1.Cells(a, 1) = h) Then
                Sheet2.Cells(b, 1) = i
                Sheet2.Cells(b, 2) = Sheet1.Cells(a, 5)
                Sheet2.Cells(b, 3) = h
            ...
            For h = 1 To HUC_num '流域数
                For j = V_start To V_num  '植被类型数
                    If (Sheet2.Cells(a, 4).Value = h And Sheet2.Cells(a, 5).Value = j) Then    '判断植被类型在的单元格值是否正确
                        ...
                        Sheet3.Cells(b, 2) = Sheet2.Cells(a, 2)
                    ...
                    Else
                        ...
                        Sheet3.Cells(b, 3) = 0  '月LAI所在的行
                        Sheet3.Cells(b, 2) = k  '月所在的行
...
'自定义排序："流域号"---"年"---"月",升序
    ...
    ActiveWorkbook.Worksheets("Sheet3").Sort.SortFields.Add Key:=Range("D2:D1000000"), _
        SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
    ActiveWorkbook.Worksheets("Sheet3").Sort.SortFields.Add Key:=Range("A2:A1000000"), _
        SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
    ActiveWorkbook.Worksheets("Sheet3").Sort.SortFields.Add Key:=Range("B2:B1000000"), _
        SortOn:=xlSortOnValues, Order:=xlAscending, DataOption:=xlSortNormal
